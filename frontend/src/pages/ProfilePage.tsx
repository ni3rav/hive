...
import { LogOut, KeyRound, BarChart, History, ChevronRight, Edit } from "lucide-react";
import { EditProfileForm } from "@/components/EditProfileForm";
import { useState } from "react";

function ProfileActionRow({ label, icon: Icon }: { label: string; icon: React.ElementType }) {
  return (
    <button className="group flex w-full items-center justify-between rounded-lg border bg-background p-4 text-left font-medium hover:bg-accent">
      <div className="flex items-center gap-3">
        ...
        <span className="transition-colors group-hover:text-accent-foreground">{label}</span>
      </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={handleLogout} disabled={logoutMutation.isPending}>
                      {logoutMutation.isPending ? "Logging out..." : <><LogOut className="mr-2 h-4 w-4" />Logout</>}
                    </Button>
                  </div>
                </div>
              </CardHeader>
